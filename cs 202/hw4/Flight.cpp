...
* Section: 1
* Assignment: 4
* Description: this is implementation of flight object which keeps destination, starting aitport, cost and next flight.
*/
...
Flight::Flight(int too, int costt, int fromm){
    to = too;
    cost = costt;
    next = nullptr;
    from = fromm;
...
void Flight::setNext(Flight* newNext){
    next = newNext;

}
int Flight::getDestination(){
    return to;
}
...
int adjList::shortestPath(int startingAirport, int destination, bool print){
    ...
    if(visitedAirports[destination]){
        ...
        if(print){
            ...
            std::cout<<"total cost: "<<knownDistances[destination]<<std::endl;
        }
    }
    else{
        if(print){
...
void adjList::minimize(){
    ...
    for(int i = 1; i<numOfAirports; i++){
        if(this->shortestPath(0, i, false) > 2000){
